...
namespace din {
	bool parse_commandline (int parArgc, char* parArgv[], po::variables_map& parVarMap) {
		...
		positional_options.add_options()
			("query-command", po::value<std::vector<std::string>>(), "Query command")
		;
		...
		if (dinlib::manage_common_commandline(std::cout, ACTION_NAME, "[options...] <query command>", parVarMap, {std::cref(desc), std::cref(set_options)})) {
			return true;
		}
